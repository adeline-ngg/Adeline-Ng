...
interface StorySelectProps {
  ...
  onLessonsSummaryClick?: () => void;
}

const StorySelect: React.FC<StorySelectProps> = ({ 
  userProfile, 
  onStorySelect, 
  onProfileDeleted, 
  onProfileUpdated,
  onTutorialClick,
  onLessonsSummaryClick
}) => {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-amber-100 to-orange-200 p-4 sm:p-6 md:p-8 relative">
      {/* Decorative background pattern */}
      <div className="absolute inset-0 opacity-5 pointer-events-none">
        <div className="absolute top-10 left-10 w-32 h-32 border-4 border-amber-300 rounded-full"></div>
        <div className="absolute top-32 right-20 w-24 h-24 border-4 border-amber-400 rounded-full"></div>
        <div className="absolute bottom-20 left-1/4 w-20 h-20 border-4 border-amber-300 rounded-full"></div>
        <div className="absolute bottom-32 right-1/3 w-28 h-28 border-4 border-amber-400 rounded-full"></div>
      </div>
      <header className="max-w-4xl mx-auto flex items-center justify-between mb-8">
        <div>
          <h1 className="text-3xl sm:text-4xl font-bold text-stone-800 font-script">Welcome, {userProfile.name}!</h1>
          <p className="text-stone-600 mt-1 font-serif">Choose a story to begin your interactive experience.</p>
        </div>
        <div className="flex items-center gap-4">
          {/* Lessons Summary Quick Access */}
          <button
            onClick={() => onLessonsSummaryClick?.()}
          ...
          <HamburgerMenu
            ...
            onLessonsClick={() => onLessonsSummaryClick?.()}
      ...
      <div className="max-w-4xl mx-auto grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {STORIES.map((story) => {
          ...
          return (
            <div key={story.id} className="bg-white/80 backdrop-blur-sm rounded-2xl shadow-lg overflow-hidden border-2 border-amber-200 flex flex-col group transition-all duration-300 hover:shadow-2xl hover:-translate-y-1 hover:border-amber-300">
              ...
              <div className="p-6 flex flex-col flex-grow relative">
                ...
                <h2 className="text-2xl font-bold font-script text-stone-800">{story.title}</h2>
                <p className="text-stone-600 mt-2 mb-4 flex-grow font-serif">{story.description}</p>
